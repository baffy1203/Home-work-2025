...
function getUserWithMaxBalance(): User {
  return users.reduce((maxUser, currentUser) => {
    return parseBalance(currentUser.balance) >
      parseBalance(maxUser.balance)
      ? currentUser
      : maxUser;
  });
}

console.log(getUserWithMaxBalance());
